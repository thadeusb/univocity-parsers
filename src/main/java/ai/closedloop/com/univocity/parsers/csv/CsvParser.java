 ...
 ******************************************************************************/
package ai.closedloop.com.univocity.parsers.csv;

import ai.closedloop.com.univocity.parsers.common.AbstractParser;
import ai.closedloop.com.univocity.parsers.common.CommonParserSettings;
import ai.closedloop.com.univocity.parsers.common.TextParsingException;
...
import static ai.closedloop.com.univocity.parsers.csv.UnescapedQuoteHandling.*;
...
public final class CsvParser extends AbstractParser<CsvParserSettings> {
				...
				if (ch == quote) {
						...
						if (value != null) {
							...
							try {
								...
								if (ch == delimiter) {
									...
									} catch (ai.closedloop.com.univocity.parsers.common.input.EOFException e) {
										output.emptyParsed();
										return;
									}
								}
					...
					if (value != null) {
						...
						if (len != -1) {
							...
							// Handle escaping delimiters if it is defined.
							if (delimiterEscape == '\0') {
								ch = output.appender.appendUntil(ch, input, delimiter, newLine);
							} else {
								ch = output.appender.appendUntilEscaped(ch, input, delimiterEscape, delimiter, newLine);
							}
						} else {
	...
	public final void updateFormat(CsvFormat format) {
		...
		delimiterEscape = format.getDelimiterEscape();
