 ...
 ******************************************************************************/
package ai.closedloop.com.univocity.parsers.common.input;
...
public class DefaultCharAppender implements CharAppender {
	...
	public char appendUntilEscaped(char ch, CharInput input, char escape, char stop1, char stop2) {
		while(true) {
			// If its an escaped STOP character, then ignore it and strip out the escape.
			if (ch == escape) {
				char nextCh = input.nextChar();
				if (nextCh == stop1 || nextCh == stop2) {
					// Strip out the escape character, uncomment line below to include escape char.
					//chars[index++] = ch;
					chars[index++] = nextCh;
					ch = input.nextChar();
				} else {
